...
export default {
  ...
  methods: {
    async confirmDelete() {
      try {
        await api.post(`${BASE_URL}products/delete/${this.productId}`)
        this.deleteStatus = 'Produk berhasil dihapus!'
        this.$emit('deleted')
        setTimeout(() => {
          this.closeModal()
        }, 1000)
      } catch (error) {
        this.deleteStatus = error.response?.data?.message || 'Gagal menghapus produk!'
