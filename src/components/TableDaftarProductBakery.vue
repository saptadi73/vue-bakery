<template>
  <div class="product-table-container">
    ...
    <table class="product-table" v-if="!isMobile">
      ...
      <tbody>
        <tr v-for="product in filteredProducts" :key="product.code">
          ...
          <td>{{ product.code }}</td>
          <td>
            <input type="date" v-model="orderDates[product.code]" class="date-input" />
          </td>
          <td>
            <span
              :class="[
    ...
    <div class="product-card-list" v-else>
      <div class="product-card" v-for="product in paginatedProducts" :key="product.code">
        <div class="img-edit-wrapper">
          <img :src="product.image" :alt="product.name" class="product-image" />
          <button class="edit-img-btn" @click.prevent="openEditImageModal(product)">✎</button>
        </div>
        <div class="product-info">
          ...
          <div class="product-code">Kode: {{ product.code }}</div>
          <div class="product-date">
            <label class="date-label">Tanggal:</label>
            <input type="date" v-model="orderDates[product.code]" class="date-input" />
          </div>
    ...
    <div class="pagination-bar" v-if="isMobile && totalPages > 1">
...
export default {
      ...
      products: [
        {
          code: 'BK001',
          name: 'Roti Tawar',
          image: new URL('@/assets/images/yummy_cake.png', import.meta.url).href,
        },
        {
          code: 'BK002',
          name: 'Donat Coklat',
          image: new URL('@/assets/images/cewek_cantik1.png', import.meta.url).href,
        },
        {
          code: 'BK003',
          name: 'Croissant',
          image: new URL('@/assets/images/a_class.jpg', import.meta.url).href,
        },
  ...
  computed: {
    ...
    totalPages() {
      return Math.max(1, Math.ceil(this.filteredProducts.length / 4))
    },
    paginatedProducts() {
      const start = (this.currentPage - 1) * 4
      return this.filteredProducts.slice(start, start + 4)
    },
  },
  // ...existing code...
  mounted() {
